...
def stand(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None):
  ...
  task = HumanoidIG(move_speed=0, pure_state=False, random=random)
...
def walk(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None):
  ...
  physics = Physics.from_xml_string(*get_model_and_assets())
  task = HumanoidIG(move_speed=_WALK_SPEED, pure_state=False, random=random)
  environment_kwargs = environment_kwargs or {}
  return control.Environment(
      physics, task, time_limit=time_limit, control_timestep=_CONTROL_TIMESTEP,
      **environment_kwargs)
...
def run(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None):
  ...
  physics = Physics.from_xml_string(*get_model_and_assets())
  task = HumanoidIG(move_speed=_RUN_SPEED, pure_state=False, random=random)
  environment_kwargs = environment_kwargs or {}
  return control.Environment(
      physics, task, time_limit=time_limit, control_timestep=_CONTROL_TIMESTEP,
      **environment_kwargs)
...
def run_pure_state(time_limit=_DEFAULT_TIME_LIMIT, random=None,
  ...
  """Returns the Run task."""
  physics = Physics.from_xml_string(*get_model_and_assets())
  task = HumanoidIG(move_speed=_RUN_SPEED, pure_state=True, random=random)
  environment_kwargs = environment_kwargs or {}
  return control.Environment(
      physics, task, time_limit=time_limit, control_timestep=_CONTROL_TIMESTEP,
      **environment_kwargs)
...
class HumanoidIG(base.Task):
  ...
  def __init__(self, move_speed, pure_state, random=None):
    ...
    self._terminate_at_height = 0.8
    print("Termination heigth = ", self._terminate_at_height)
    super(HumanoidIG, self).__init__(random=random)
  ...
  def initialize_episode(self, physics):
    ...
    super(HumanoidIG, self).initialize_episode(physics)
